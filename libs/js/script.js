...
$(document).ready(function () {
  $("#tabla_especialidad").DataTable({
    ...
    procesing: true,
    serverSide: true,
    ajax: "http://127.0.0.1/sigeser/index.php?page=listarEspecialidades",
    pageLength: 10,
    columnDefs: [
      {
        orderable: false,
...
if ((agregar_especialidad = document.getElementById("agregar_especialidad"))) {
  ...
  function agregarEspecialidad() {
    let nombre_especialidad = document.getElementById(
      "nombre_especialidad"
    ).value;
    let estatus_especialidad = document.getElementById(
    ...
    $.ajax({
      ...
      .done(function (response) {
        if (response.data.success == true) {
          ...
          $("#tbl_persona").DataTable().ajax.reload();
          contenedor_formulario_persona.setAttribute("style", "display: none;");
          contenedor_datos_persona.removeAttribute("style");
        } else {
          Swal.fire({
